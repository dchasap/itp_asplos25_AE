...
def plot_reuse_distance(df, tags, output_file):
    ...
    last_bucket_freq_1 = round(df.loc[df['tag'] == "TXVC"].tail(1)['frequency'].item(), 2)
    last_index = df.loc[df['tag'] == "TXVC"].index[-1]
    df = df.drop(index=last_index)
		#last_bucket_freq_2 = round(df.tail(1)['frequency'].item(), 2)
    #df = df[:-1]
		
    last_bucket_freq_2 = round(df.loc[df['tag'] == "TXVC-DOA"].tail(1)['frequency'].item(), 2)
    last_index = df.loc[df['tag'] == "TXVC-DOA"].index[-1]
    df = df.drop(index=last_index)

    ax = df.loc[df['tag'] == "TXVC"].plot.area(x='reuse_distance', y='frequency', color='blue', alpha=0.3, label='TXVC')
    ax = df.loc[df['tag'] == "TXVC-DOA"].plot.area(x='reuse_distance', y='frequency', color='red', alpha=0.3, label='TXVC-DOA', ax = ax)
