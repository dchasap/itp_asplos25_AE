...
def export_env_variables(benchmark, sim_name, dump_dir, enviromental_variables):
  ...
  for var in enviromental_variables:
    if (var == "TXVC_MEMORY_TRACE_PATH"):
      ...
      export_cmd += "export " + var + "=" + dump_dir + "/" + benchmark + "_" + sim_name + "_address_access_stats\n"
    elif (var == "SET_ACCESS_FILENAME_PREFIX"):
      export_cmd += "export SET_ACCESS_FILENAME_PREFIX=" + dump_dir + "/" + benchmark + "_" + sim_name + "_set_access_stats\n"
    else:
...
def run_simulation_batch(root_dir, trace_dir, dump_dir, sim_name, exp_name, workload_name, sim_config, enviromental_variables, parallel_run, debug_run):
  ...
  if debug_run:
    ...
    #debug_flags = 'gdb -batch -ex "run" -ex "bt" --args' # temporary disabled until ptrace is patched
    debug_flags = ''
    job_queue = 'gp_bsccs'
